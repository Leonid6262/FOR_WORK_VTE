...
CIADC::CIADC(CADC_STORAGE& adstr, LPC_ADC_TypeDef* pIadc) : adstr(adstr), pIadc(pIadc) {
  pIadc->CNR = (((PeripheralClock / OSC_CLK) - 1) << SB_CLKDIV) | PDN;
};
...
StatusRet CIADC::measure_5V() {
  ...
  pIadc->CNR &= CLEAR;
  pIadc->CNR |= ((1UL << CH_P5) | START);
  unsigned int prev_TC0 = SysT::TC();
  while (!(pIadc->GDR & DONE)) {
  ...
  P5_cur = K_P5 * ((pIadc->DR[CH_P5] >> 4) & 0xFFF);
  ...
  N5_cur = (K_N5 * ((pIadc->DR[CH_N5] >> 4) & 0xFFF)) - (3 * P5_A);
